...
from demo_utils.inference_engine import OpenaiEngine,AwsEngine
...
async def main(config, base_dir) -> None:
    ...
    if "nova" in config["openai"]["model"]:
        generation_model = AwsEngine(**openai_config, )
    else:
    ...
    # with open("/local/scratch/xue.681/research/Auto_eval/mind2web_live_subsamples_woranker/mind2web_live_subsamples_human_eval_16.json", "r") as f_human:
    #     human_eval_results = json.load(f_human)

    # human_eval_index = [i["task_id"] for i in human_eval_results]
    temp_stop_dir = ['9aec1bbb-e7ac-45e4-bcbd-ec2cee25af1a', '5419702c-1530-441b-bdef-409711cf6a13', '7d6b41a1-ffb3-49a3-8ad8-71b8f3d5f5a6', '72b8fc77-1373-4dc1-81ef-aac757c7915c', '5a08f2d9-799b-4eff-83ce-b27548fd526a', '4c224b36-5550-4129-accd-de155481eb4d', '81c87171-97f5-485d-8014-7723b3f98617', '84c1847c-a0f3-4062-b778-625578a0fba7', '0c470ef9-3409-48ec-8bb7-68ba580cfc9b', '6accc593-0fed-47f4-a021-42eed5f94ee7', 'f36b49c8-7e36-4d48-b736-518d5a617a8f', '2f096936-2d36-40bc-8fbd-8f91785410ea', '158027d3-05c9-4a7b-af96-43625c84e0e2', '5635a86d-5ecc-4a65-b4f9-5f9b661a7303', 'a8318271-1ec4-4973-9864-1aa4041441d0', '24e104ae-aa71-40b2-8048-b69027f9652b', '924476f8-37f9-42e2-bedf-64eeac0ce8b6', 'cd5753f3-4487-40e5-aa8b-50ac687a4631', '47efde90-81c4-4845-ab07-1eb8e7a9d11a', 'f7635bd8-161b-40a8-a5d5-8d76f2ba4810', '1da50745-6c23-4fad-a7e7-2e43b4993137', '6807ef3b-90d9-4592-b97f-8db60398985a']
    temp_stop_dir = ["44"]
    for single_query_task in query_tasks:
        if single_query_task["task_id"] not in temp_stop_dir:
            continue
        # if single_query_task["reference_length"] < 11:
        #     continue
            ...
            while not complete_flag:
                ...
                for multichoice_i in range(0, num_choices, step_length):
                    ...
                    try:
                        ...
                        await session_control.active_page.screenshot(path=os.path.join(main_result_path, 'trajectories',f'{time_step}_full.jpg'), full_page=True,
                                                                     type='jpeg', quality=100, timeout=20000)
